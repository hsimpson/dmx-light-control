...
function makeFh(read: ReadMock): fs.FileHandle {
  return {
    read,
...
function readMock(): ReadMock {
  return vi.fn<(buf: Buffer) => Promise<ReadResult>>();
}
...
describe('DmxSnifferService', () => {
  it('startSniffer reads packets, reassembles a 513-byte frame and logs', async () => {
    ...
    const logSpy = vi.spyOn((service as unknown as WithLogger).logger, 'log').mockImplementation(() => undefined);
    const errorSpy = vi.spyOn((service as unknown as WithLogger).logger, 'error').mockImplementation(() => undefined);
    const logged: string[] = [];
    vi.spyOn((service as unknown as WithLogger).logger, 'log').mockImplementation((...a: unknown[]) => {
  ...
  it('startSniffer ignores packets that do not target the requested device/endpoint/type', async () => {
    ...
    const _packet = Buffer.concat([header, Buffer.from([0, 1, 2, 3])]);

    const read = readMock().mockRejectedValueOnce(new Error('eof'));
    const fh = makeFh(read);
    const openSpy = vi.spyOn(fs, 'open').mockResolvedValue(fh);
  ...
  it('startSniffer continues after a zero-byte read', async () => {
    ...
    const read = readMock()
      .mockImplementationOnce(async (_buf: Buffer) => Promise.resolve({ bytesRead: 0, buffer: _buf }))
      .mockImplementationOnce(async (buf: Buffer) => {
        packet.copy(buf, 0);
        return Promise.resolve({ bytesRead: packet.length, buffer: buf });
      })
