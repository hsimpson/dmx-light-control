...
@Injectable()
export class DmxSnifferService {
  // This service will handle the logic for sniffing DMX data frames from USB devices.
  // It will be used by the DmxSnifferCommand to start the sniffer and process the data.

  private readonly logger = new Logger(DmxSnifferService.name);

  public async startSniffer(bus: number, address: number): Promise<void> {
    const devicePath = `/dev/usbmon${bus}`;
    this.logger.log(`🚀 Starting DMX Sniffer on ${devicePath}...`);
